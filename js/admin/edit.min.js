...
p=l.locks||!(a.parent()&&a.parent().model.get("locked")),h=a.closest(d.view.element.Grid),g=a.closest(d.view.element.Container),f=a.closest(d.view.element.Row),k=a.closest(d.view.element.Cell),m=a.model.get("visibility_"+this.whichView())||"";can_edit=!e&&c;can_clone=!e&&c&&p;can_delete=!e&&c&&p;can_lock=!0;c=!e;can_create_template=!e;can_update_template=!e&&!!a.model.get("template");e=l.locks||!h.model.get("locked");can_add_row=!!g&&(l.locks||!g.model.get("locked"));can_add_cell=!!f&&(l.locks||!f.model.get("locked"));
can_add_widget=!!k&&(l.locks||!k.model.get("locked"));this.$(".item-action.lock").prop("checked",!!a.model.get("locked"));this.$('[name="set-visibility"]').each(function(a,c){var e=b(this);e.prop("checked",e.val()==m)})}this.$(".item-action.edit").prop("disabled",!can_edit);this.$(".item-action.clone").prop("disabled",!can_clone);this.$(".item-action.delete").prop("disabled",!can_delete);this.$(".item-action.lock").prop("disabled",!can_lock);this.$(".create-template").prop("disabled",!can_create_template);
...
this.toolbar.update()},editItem:function(a){a&&a.preventDefault();if(b("body").hasClass("grid-modal-open"))return!1;var c=this;a=this.getSelected();for(var e=this.getNextItem(a),g=this.getPrevItem(a),h=new f({controller:this,next:!!e,prev:!!g}),l=k.editors[a.getClassName().toLowerCase()],n,q=[],m=a,r;m&&!m.is(d.view.element.Grid);)r=m.getTitle(),q.unshift(r),m=m.parent();n=new d.view.EditDialog({title:q.join(" \u203a "),controller:this,model:a.model,item:a,editor:l});n.on("done",function(){c.grid.hasChanged();
h.close()},h);h.on("close",function(a){n.dismiss();c.getSelected().$el.focus()}).on("next",function(a){n.applyChanges();c.grid.hasChanged();h.close();c.setSelected(e);c.editItem(a)}).on("prev",function(a){n.applyChanges();c.grid.hasChanged();h.close();c.setSelected(g);c.editItem(a)});h.content(n).open();h.$el.focus();return!1},deleteItem:function(a){a&&a.preventDefault();a=this.getSelected();var c=a.parent();c.getClass();a.remove();c.model.items.remove(a.model);this.grid.hasChanged();return!1},reFocus:function(){var a=
this.getSelected();a&&a.reFocus()},setVisible:function(a){var c=this.getSelected(),b="visibility_"+this.toolbar.whichView();c&&(c.model.set(b,a),c.reFocus())},createTemplate:function(){var a=this,c=this.getSelected(),b=new d.model.GridTemplate,f=new d.view.ui.Prompt({title:g.TemplateName,description:g.CreateTemplateDescription,defaultValue:g[c.getClassName()]+" "+g.Template});f.on("confirm",function(e){e=f.getValue();var g=d.utils.sanitizeTitle(e),h=c.getClassName().toLowerCase();b.set("name",e);
b.set("slug",g);b.set("type",h);b.set("data",c.model.toJSON());b.once("sync",function(){var e=b.get("slug");b.set("id",e);c.model.set("template",e);d.templates[h].add(b);toolbar.update();a.grid.hasChanged()});b.save();c.reFocus()},this).open();return!1},updateTemplate:function(){var a=this.getSelected(),b=a.getClassName().toLowerCase(),e=a.model.get("template");template=d.templates.get(b,e);template.set("data",a.model.toJSON());template.save();a.reFocus();return!1},openTemplateManager:function(){var a=
this,b=new wp.media.view.Modal({controller:this}),e=this.getSelected(),f=new d.view.ManageTemplatesDialog({controller:this,title:g.ManageTemplates});f.on("done",function(){b.close()},b);b.content(f).on("close",function(b){a.toolbar.setupTemplateSelects();e&&e.$el.focus()}).open();return!1}});_.extend(d,{getActiveEditor:function(){return active_editor},setActiveEditor:function(a){active_editor=a}})})(jQuery,window.grid);
...
model:this.model,settings:{title:"",items:a.editor.main}});_.each(a.editor.sidebar,function(a,c){_.extend(a,{name:c});var d=new n({controller:b,model:b.model,settings:a});b.inputgroups.push(d)});return this},render:function(){c.view.ui.Dialog.prototype.render.apply(this,arguments);var a=this;_.each(this.inputgroups,function(b){b.render();a.$(".grid-dialog-sidebar").append(b.$el)});this.$(".grid-dialog-content").append(this.editor.render().$el);return this},applyChanges:function(){function a(a){if(a.options.settings&&
